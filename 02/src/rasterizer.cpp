...
{   
    Vector3f p(static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f, 1.0f);
    Vector3f a(_v[0].x(), _v[0].y(), 1.0f);
    Vector3f b(_v[1].x(), _v[1].y(), 1.0f);
    Vector3f c(_v[2].x(), _v[2].y(), 1.0f);

    auto z1 = (b - a).cross(p - a).z();
    auto z2 = (c - b).cross(p - b).z();
    auto z3 = (a - c).cross(p - c).z();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int x = min_x; x <= max_x; ++x)
    {
        for (int y = min_y; y <= max_y; ++y)
        {
            ...
            auto[alpha, beta, gamma] = computeBarycentric2D(static_cast<float>(x) + 0.5f,
            ...
            if (z_interpolated < depth_buf[index])
            {
                ...
                set_pixel(Eigen::Vector3f(static_cast<float>(x), static_cast<float>(y), z_interpolated),
                          t.getColor());
            }
        }
    }
